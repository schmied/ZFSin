		...
		if (free_memory < 0 || manual_pressure > 0) {
			...
			if(to_free > 0 || manual_pressure != 0) {
				...
				if (to_free > large_amount || evicted > huge_amount)
					dprintf("SPL: %s: post-reap %lld post-evict %lld adjusted %lld "
	...
	if (rw == KSTAT_WRITE) {
		...
		if (ks->arc_zfs_arc_min.value.ui64 != zfs_arc_min) {
			...
			if (zfs_arc_min > 64<<20 && zfs_arc_min <= arc_c_max) {
				...
				dprintf("ZFS: set arc_c_min %llu, arc_meta_min %llu, zfs_arc_meta_min %llu\n",
				    arc_c_min, arc_meta_min, zfs_arc_meta_min);
				if(arc_c < arc_c_min) {
				  dprintf("ZFS: raise arc_c %llu to arc_c_min %llu\n",
					 arc_c, arc_c_min);
				  arc_c = arc_c_min;
				  if(arc_p < (arc_c >> 1)) {
				    dprintf("ZFS: raise arc_p %llu to %llu\n",
					   arc_p, (arc_c >> 1));
				    arc_p = (arc_c >> 1);
				  }
				}
		...
		if (ks->arc_zfs_arc_meta_limit.value.ui64 != zfs_arc_meta_limit) {
			...
			dprintf("ZFS: set arc_meta_limit %llu, arc_c_min %llu, zfs_arc_meta_limit %llu\n",
		...
		if (ks->arc_zfs_arc_meta_min.value.ui64 != zfs_arc_meta_min) {
		  ...
		  if (zfs_arc_meta_min >= arc_c_min) {
		    dprintf("ZFS: probable error, zfs_arc_meta_min %llu >= arc_c_min %llu\n",
...
l2arc_write_buffers(spa_t *spa, l2arc_dev_t *dev, uint64_t target_sz)
{
	 ...
	 */
	for (int xtry = 0; xtry < L2ARC_FEED_TYPES; xtry++) {
		multilist_sublist_t *mls = l2arc_sublist_lock(xtry);
