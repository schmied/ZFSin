...
void
dnode_buf_byteswap(void *vbuf, uint32_t size)
{
...
#ifndef _WIN32  // Our kmem_cache does not use KMEM_UNINITIALIZED_PATTERN
...
dnode_special_close(dnode_handle_t *dnh)
{
	...
	while (refcount_count(&dn->dn_holds) > 0) {
		delay(hz);
        if (count++ > 9) {
            dprintf("dnode: ARC release bug triggered: %p (%lld)-- sorry\n", dn,
